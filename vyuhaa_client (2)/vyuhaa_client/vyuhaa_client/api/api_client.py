...
class RelayClient(QObject):
    ...
    def __init__(self, state: AppState, parent: QObject | None = None) -> None:
        ...
        self._webrtc.offer_ready.connect(self._on_webrtc_offer_ready)
        self._webrtc.ice_ready.connect(self._on_webrtc_ice_ready)
        self._webrtc.frame_received.connect(self._on_p2p_frame)
        self._webrtc.command_response.connect(self._handle_proxy_response)
        self._webrtc.connected.connect(self._on_p2p_connected)
        self._webrtc.disconnected.connect(self._on_p2p_disconnected)
    ...
    def _handle_state_sync(self, state: dict) -> None:
        ...
        if kind == "files_list_updated":
            print("[SYNC-CLIENT] Detected files_list_updated, refreshing file list...")
            self.get_files()
            return
